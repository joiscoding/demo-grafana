...
const structuredLogger = createStructuredLogger('packages/grafana-ui/src/components/ClipboardButton/ClipboardButton');
...
<ClipboardButton
  ...
  onClipboardCopy={() => structuredLogger.log('text copied')}
>
  Copy to clipboard
</ClipboardButton>
```
