...
const structuredLogger = createStructuredLogger('packages/grafana-ui/src/components/DateTimePickers/TimeRangeInput');
...
<TimeRangeInput
  value={timeRange}
  onChange={(range) => structuredLogger.log('range', range)}
  onChangeTimeZone={(tz) => structuredLogger.log('timezone', tz)}
/>;
```
