...
export class LivePanel extends PureComponent<Props, State> {
  ...
  streamObserver: PartialObserver<LiveChannelEvent> = {
    next: (event: LiveChannelEvent) => {
      if (isLiveChannelStatusEvent(event)) {
        ...
        logStructuredInfo('public/app/plugins/panel/live/LivePanel.tsx','ignore', event);
      }
    },
  };
  ...
  async loadChannel() {
    ...
    if (!isValidLiveChannelAddress(addr)) {
      logStructuredInfo('public/app/plugins/panel/live/LivePanel.tsx','INVALID', addr);
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    if (isEqual(addr, this.state.addr)) {
      logStructuredInfo('public/app/plugins/panel/live/LivePanel.tsx','Same channel', this.state.addr);
      return;
    }
    ...
    logStructuredInfo('public/app/plugins/panel/live/LivePanel.tsx','LOAD', addr);
